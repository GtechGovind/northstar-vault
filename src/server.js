...
app.get('/api/private/sessions/:id', async (req, res, next) => {
  try {
    ...
    if (!session.exists || session.get('deleting')) return res.status(404).json({ error: 'Reflection not found.' });
    return res.json({
...
app.post('/api/private/chat', aiLimiter, async (req, res, next) => {
  try {
    ...
    const prepared = await db.runTransaction(async (transaction) => {
      ...
      if (user.get('erasing') || existing.get('deleting')) throw new RequestConflict('A deletion is in progress. Please retry afterwards.');
      if (input.sessionId && !existing.exists) return null;
      const historySnapshot = await transaction.get(sessionRef.collection('messages').orderBy('createdAt', 'desc').limit(10));
      ...
      return { epoch, history: historySnapshot.docs.reverse().map((doc) => ({ role: doc.get('role'), text: doc.get('text') })) };
    });
    if (!prepared) return res.status(404).json({ error: 'Reflection not found.' });

    const analysis = await generateReflection({ message: input.message, history: prepared.history });
    await db.runTransaction(async (transaction) => {
      const user = await transaction.get(userRef);
      const current = await transaction.get(sessionRef);
      if (user.get('erasing') || user.get('dataEpoch') !== prepared.epoch || !current.exists || current.get('deleting')) {
        throw new RequestConflict('This reflection was deleted while the reply was being generated. No reply was saved.');
      }
      transaction.create(sessionRef.collection('messages').doc(), {
...
app.delete('/api/private/sessions/:id', async (req, res, next) => {
  try {
    z.object({ confirmation: z.literal('DELETE REFLECTION') }).strict().parse(req.body);
    const id = idSchema.parse(req.params.id);
...
app.delete('/api/private/data', async (req, res, next) => {
  ...
  const userRef = db.collection('users').doc(req.user.uid);
  try {
    z.object({ confirmation: z.literal('ERASE MY VAULT') }).strict().parse(req.body);
    ...
    if (erasureEpoch) {
      await userRef.set({ dataEpoch: erasureEpoch, erasing: false }, { merge: true }).catch(() => {});
    }
    next(error);
  }
});
...
app.use((error, req, res, _next) => {
  ...
  if (error instanceof RequestConflict) return res.status(409).json({ error: error.message, requestId });
  ...
  console.error(JSON.stringify({ severity: 'ERROR', requestId, route: req.route?.path || 'unmatched', error: 'REQUEST_FAILED' }));
  const status = /not configured|invalid structured/.test(error.message) ? 503 : 500;
  return res.status(status).json({
    error: status === 503 ? 'The reflection engine is temporarily unavailable.' : 'The request could not be completed. Refresh to check your saved data before retrying.',
    requestId
  });
});
