...
const record = () => ({
  ...
  tags: ['test'],
  compass: { clarity: 3, agency: 4, energy: 2 },
  createdAt: timestamp,
  updatedAt: null,
  messages: [
    { role: 'user', text: 'नमस्ते 🌍 決定 Привет', createdAt: null },
    { role: 'assistant', text: 'Synthetic reply', createdAt: timestamp },
  ],
});
...
test('whitespace and key-order changes produce different exact-file hashes', async () => {
  ...
  const strings = [
    ...
    JSON.stringify({ sessions: data.sessions, exportedAt: timestamp }),
  ];
...
test('invalid root, dates and nested records fail without echoing private input', async () => {
  ...
  for (const field of [
    ...
    delete item[field];
    await assert.rejects(
      createPrivacyReceipt(JSON.stringify(exported([item]))),
      (error) => !error.message.includes('synthetic-private-id'),
    );
  }
  for (const change of [{ role: 'system' }, { text: 42 }, { createdAt: 'invalid' }]) {
    const item = record();
    Object.assign(item.messages[0], change);
...
test('receipt workload is bounded before parsing and after UTF-8 encoding', async () => {
  ...
  await assert.rejects(
    createPrivacyReceipt('界'.repeat(Math.floor(MAX_EXPORT_BYTES / 3) + 1)),
    /10 MiB/,
  );
});
...
test('cancellation before and during hashing rejects without returning a receipt', async () => {
  ...
  before.abort('SYNTHETIC PRIVATE');
  await assert.rejects(
    createPrivacyReceipt(raw, { signal: before.signal }),
    (error) => error.name === 'AbortError' && !error.message.includes('SYNTHETIC PRIVATE'),
  );
...
test('digest failures are sanitized rather than reported as a successful receipt', async (t) => {
  ...
  await assert.rejects(
    createPrivacyReceipt(JSON.stringify(exported())),
    (error) => /No receipt/.test(error.message) && !error.message.includes('SYNTHETIC PRIVATE'),
  );
});
