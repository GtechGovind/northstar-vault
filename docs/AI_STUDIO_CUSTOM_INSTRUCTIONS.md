...
5. **Zero hardcoded secrets.** Never place keys or service-account JSON in source, client bundles, logs, tests, Docker layers, or documentation. The contest configuration injects a pinned Gemini credential from Secret Manager into Cloud Run server-side. The existing-key preflight and runtime secret binding are now verified; explicit keyless Vertex AI mode remains available only as rollback. Firebase web configuration is public configuration and should still be runtime-injected.
