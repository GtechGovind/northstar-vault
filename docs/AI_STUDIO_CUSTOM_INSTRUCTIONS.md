...
5. **Zero hardcoded secrets.** Never place keys or service-account JSON in source, client bundles, logs, tests, Docker layers, or documentation. Mount the Gemini API key from Google Cloud Secret Manager into Cloud Run. Firebase web configuration is public configuration and should still be runtime-injected.
