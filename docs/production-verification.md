...
Baseline verified against Cloud Run revision `northstar-vault-00007-fjl` on 27 August 2026. The security repair below is separately tested; its deployment status must be recorded after promotion.
...
- Production two-Google-account isolation smoke test (local emulator equivalent passed).
